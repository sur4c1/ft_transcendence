...
export class MuteController {
	...
	): Promise<Mute>
	{
		...
		if (!channel)
			throw new HttpException('Channel not found', HttpStatus.NOT_FOUND);
		let my_membership = await this.membershipService.findByUserAndChannel(me.dataValues.login, channelName);
		if (!my_membership)
			throw new HttpException('User can\'t mute if he\'s not in channel', HttpStatus.FORBIDDEN);
		let user_membership = await this.membershipService.findByUserAndChannel(login, channelName);
		if (!user_membership)
		...
		if (user_membership.dataValues.isAdmin && channel.dataValues.owner != user)
			throw new HttpException('Cannot mute channel admin', HttpStatus.FORBIDDEN);
		let old_mute = await this.muteService.findByLoginAndChannel(login, channelName);
	...
	): Promise<number>
	{
		...
		if (!ret)
			throw new HttpException('Mute not found', HttpStatus.NOT_FOUND);
		let me = await this.userService.findByLogin('me' /* TODO: session */);
		let my_membership = await this.membershipService.findByUserAndChannel(me.dataValues.login, ret.dataValues.channelName);
		if (!my_membership)
			throw new HttpException('User can\'t mute if he\'s not in channel', HttpStatus.FORBIDDEN);
		if (!my_membership.dataValues.isAdmin)
					throw new HttpException('You can\'t do this', HttpStatus.FORBIDDEN);
