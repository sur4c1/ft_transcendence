...
export class ModifierService {
	...
	async findAll(): Promise<Modifier[]> {
		try {
			return await this.modifierRepository.findAll<Modifier>({
				include: [{ all: true }],
			});
		} catch (error) {
	...
	async findById(id: number): Promise<Modifier> {
		try {
			return await this.modifierRepository.findOne<Modifier>({
				where: { id: id },
				include: [{ all: true }],
			});
		} catch (error) {
	...
	/**
	 * @brief	Find modifiers by their ids with sequelize
	 * @param	{number[]}		modifierIds
	 * @return	{Modifier[]}	List of modifiers
	 * @throws	{HttpException}	500 if an error occured
	 */
	async findByIds(modifierIds: number[]): Promise<Modifier[]> {
		try {
			let ret = [];
	...
	async update(modifierDto: ModifierDto): Promise<number> {
		try {
			return await this.modifierRepository.update<Modifier>(modifierDto, {
				where: { id: modifierDto.id },
			})[0];
		} catch (error) {
			throw new HttpException(error, HttpStatus.INTERNAL_SERVER_ERROR);
		}
